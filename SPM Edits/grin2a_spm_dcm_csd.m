function DCM = grin2a_spm_dcm_csd(DCM)
...
% OPRIGINAL:    DCM.M.FS = 'spm_fs_csd';
DCM.M.FS = 'grin2a_spm_fs_csd';                                                 % grin2a Edit: Change feature selection to custom function
DCM.M.g  = 'spm_gx_erp';
DCM.M.f  = f;
DCM.M.x  = x;
...
%--------------------------------------------------------------------------
% y     = spm_fs_csd(DCM.xY.y,DCM.M);                                       % grin2a Edit: Change feature selection to custom function               
y       = grin2a_spm_fs_csd(DCM.xY.y,DCM.M);
for i = 1:length(y)
    m      = spm_length(y{i});
    Q{i,i} = speye(m,m);
end
